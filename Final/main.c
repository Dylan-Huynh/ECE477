...
uint8_t randomMulti(int n) {
	if (n == 4){ //will be 5 in final, will be 0x1f
		return 0xf;
	}
	if (n == 3) {
		int num = rand() % 4;
		return (0xf & ~ 1 < num); // This picks a number between 0-3, then takes that target out will be 4 in final
	}
	if (n == 2) { //change these to
		int num1 = rand() % 4;
		int num2 = rand() % 4;
	 ...
	 */
		...
		int pick = targets;
		while (pick == targets) pick = ((rand() % (4)) + 1);
		return 0x1 < pick;
	}
}
